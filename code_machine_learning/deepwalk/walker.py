...
class RandomWalker:
    def __init__(self, G, p=1, q=1) -> object:
        """
        :rtype: object
        :param G:
    ...
    def deepwalk_walk(self, walk_length, start_node):
        ...
        walk = [start_node]  # 开始节点

        while len(walk) < walk_length:  # walk_length 序列长度
            cur = walk[-1]
            cur_nbrs = list(self.G.neighbors(cur))  # 从当前节点的邻居中随机采样节点
            if len(cur_nbrs) > 0:  # 采样到
                walk.append(random.choice(cur_nbrs))  # 随机选择一个，放入到节点序列中
            else:
                break
        return walk
